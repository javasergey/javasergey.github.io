...
<?php 
	...
	$resultt = mysqli_query($connection1, " 
			SELECT id, title, image, text, date, time, category_id, user FROM articles1
			WHERE id = '$id'
			");
	...
	if(isset($_POST['save'])){
		...
		mysqli_query($connection1, " UPDATE articles1 SET `title`='$title', `text`='$text', `user`='$user', `image`='$image', `category_id`='$category_id' WHERE `id`='$id'");
...
<form action="edit.php?id=<?php echo $id; ?>" method="post" style="margin-left: 50px;"><br>
	Название статьи<br>
	<input type="text" name="title" value="<?php echo $row['title'] ?>"><br>
	Категория статьи<br>
	<input type="text" name="category_id" value="<?php echo $row['category_id'] ?>"><br>
	Текст статьи<br>
	<textarea  cols="100" rows="7" name="text" id="editor" style="resize: none;"><?php echo $row['text'] ?></textarea><br>
